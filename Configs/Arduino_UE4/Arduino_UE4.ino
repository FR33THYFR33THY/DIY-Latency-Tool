...
#include <Mouse.h> // mouse to send clicks
#include <Keyboard.h> // keyboard to send keys
...
void setup()
{
  ...
  OLED_1.setCursor(40, 50); OLED_1.print("Arduino");
  OLED_1.sendBuffer();
  ...
  OLED_2.setCursor(40, 50); OLED_2.print("UE4");
  OLED_2.sendBuffer();
...
void loop()
{
  while (digitalRead(reset) == LOW) // reset button
  {
    ...
    Keyboard.write('b'); // send keyboard b key
   }
 ...
 while (digitalRead(hold) == LOW && digitalRead(reset) == HIGH) // measurement button
  {      
     ...
     Mouse.click(); // send mouse left click
