...
const byte photodiode = A3, hold = 9, reset = 8, mouse=A10, keyboard=A1;
...
void setup()
{
  ...
  OLED_1.setCursor(40, 20); OLED_1.print("");
  ...
  OLED_2.setCursor(40, 20); OLED_2.print("");
  OLED_2.setCursor(40, 50); OLED_2.print("Manual");
  OLED_2.sendBuffer();
...
void loop()
{
 ...
 while (digitalRead(hold) == LOW && digitalRead(reset) == HIGH) // measurement button
  {
     float mouse_state = analogRead(mouse); // read mouse switch voltage
     float mouse_low = (mouse_state*0.7); // lower threshold for mouse switch voltage
     float mouse_high = (mouse_state*1.3); // upper threshold for mouse switch voltage

     while (((mouse_low < mouse_state) && (mouse_state < mouse_high)) && digitalRead(hold) == LOW ) // loop while mouse value is in range and hold button is pressed
      {
       mouse_state = analogRead(mouse); // read value from mouse
      }

    if ( digitalRead(hold) == LOW ) // check hold button to avoid getting stuck
    {   
